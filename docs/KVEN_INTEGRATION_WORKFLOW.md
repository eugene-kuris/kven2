...
The runner writes `result-manifest.json` with requested model (nullable), actual runtime model, actual reasoning effort, reported token usage (nullable), exit and final status, repository and branch identities, baseline and feature heads, commits, changed files, tests, diff-check and secret-scan results, network declaration, embedded deployment contract, requested services/backups/migrations, and package path. Runtime facts are parsed from the Codex startup stream; a requested/actual model conflict forces `FAIL`. Prohibited network use is recorded as `false`; allowed but unobservable use is `null` with an explanation. `result-summary.md` is the canonical human entry point. Legacy text artifacts remain available, but schema 1.0 manifests are deliberately not integration-eligible under the stricter inspector.

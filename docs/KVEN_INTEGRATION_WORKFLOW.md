...
A command item is an object containing a non-empty argv-array `command`, plus optional `name` and `timeout`; readiness and fatal-log items may also declare `cwd`. Shell strings and literal password/secret/token/API-key/access-token/Authorization/Bearer/private-key values are rejected without reflecting their values; commands must refer indirectly to protected environment or configuration sources. All three rollback guarantees must be `true`. A migration check declares `database`, application migration `command`, explicit `application_smoke_command`, and optional `cwd`, `database_env`, `idempotent`, `timeout`, and `verification_sql`. Both commands receive the disposable database path through the named environment variable. A temporary-table probe is not treated as application verification.
...
Inspect and dry-run are read-only. Production defaults to the trusted `/opt/kven2` boundary and rejects any manifest redirect. `--repository` is an explicit operator/test-fixture override, not a value inferred from the manifest. Inspect independently recomputes the exact baseline-to-feature commit set and changed paths, compares them to the manifest, reruns `git diff --check`, scans exact changed content offline, and verifies that the recorded feature commit object and recorded branch exist. The branch may subsequently move and the operator's feature worktree may be dirty: neither mutable source is trusted for validation or merge. Dry-run prints the exact feature-validation context, intended backups, exact merge commit, staged-main tests, restarts, readiness, and fatal-log checks without creating a worktree or changing Git objects.

## Stage and acceptance

Stage takes a per-repository lock and durable active-run marker, creates a run ID and evidence directory, and persists state before every executable phase. Before backups or live mutation, it creates a uniquely named sibling directory controlled by the integration tool and adds a detached Git worktree by the exact recorded feature object ID. It never reuses the result package's worktree path or the operator's Codex worktree. Every `pre_merge_tests` argv runs with that exact checkout as `cwd` (with Python bytecode writes disabled). Before and after each command, evidence records the resolved checkout path, exact HEAD, detached/attached branch state, index-entry fingerprint, complete porcelain status including untracked paths and its SHA-256, and symlink fingerprint. It also records argv, timing, exit code, bounded output, and a full redacted log. Any HEAD, branch, index, tracked, untracked, or symlink difference is an automated failure before production mutation.

The validation checkout is removed with `git worktree remove --force`, followed by `git worktree prune` and an independent registration/path check on success and failure paths. Command evidence is persisted before cleanup. Creation, fingerprint, evidence, command, cleanup, prune, or stale-registration failure is `AUTOMATED_CHECK_FAILED` with `live_mutation_started=false`; it performs no backup, merge, reset, service action, or push. After successful cleanup, stage reverifies the exact clean production branch, HEAD, origin/main, index and status, then creates declared protected backups.

Migration validation uses a second tool-controlled disposable detached worktree at the same exact recorded feature object ID. Migration and application-smoke commands may retain the production interpreter declared by the contract, while repository-relative scripts and modules resolve from this immutable checkout. The mutable Codex result worktree and operator worktrees are never executable inputs because either may move or contain uncommitted content.

For these pre-merge checks, an omitted or relative `cwd` is rooted in the trusted migration checkout. An absolute `cwd` equal to the production repository root, or beneath it, maps to the equivalent checkout root or subdirectory. Relative `..` traversal, missing mapped directories, and mapped symlink escapes are refused. An unrelated absolute path is left unchanged rather than silently redirected.

The migration checkout receives exact-HEAD, detached-state, clean index/worktree, untracked-file, and symlink fingerprints before and after every migration attempt and application smoke command, and again after all checks. Python bytecode writes are disabled. Evidence is persisted, then the worktree is force-removed, registrations are pruned, and path and registration removal are independently verified before the final preflight and exact-SHA merge. Creation, fingerprint, command, mutation, cleanup, or prune failure remains `AUTOMATED_CHECK_FAILED` with `live_mutation_started=false`: no merge, service action, live database migration, or push occurs. Cleanup runs on success and failure.

Post-rollback readiness and fatal-log verification may also depend on helpers introduced by the feature that Git restoration correctly removes from production. After production is back at the recorded baseline, the runner therefore executes these verifiers from another tool-controlled disposable detached checkout of the exact recorded feature SHA. Repository-root and safe repository-subpath `cwd` values, and absolute argv paths beneath the production repository, map to the corresponding exact-feature checkout paths; traversal, missing paths, and symlink escapes are refused. Unrelated absolute paths are not silently remapped. The production interpreter may still be used, and `KVEN_PRODUCTION_REPOSITORY` identifies the restored runtime/state boundary so verifier code does not mistake its disposable source checkout for production.

Rollback-verification evidence records the exact feature SHA, detached clean fingerprints before and after each command, production and execution paths, argv, output artifacts, failures, cleanup, and worktree pruning. Mutation, exact-SHA mismatch, unsafe paths, command failure, or cleanup failure prevents terminal `ROLLED_BACK` status while leaving the baseline restored. Durable Git, backup, and service restoration milestones make an interrupted rollback resumable: a retry verifies the already-restored Git state, does not repeat completed backup restoration or service churn, and reruns every required rollback verifier from a fresh trusted checkout. Earlier failure evidence remains in the run directory; success is recorded only after verification passes and cleanup completes.

The complete order is exact-feature test checkout and cleanup → protected backup → exact-feature migration checkout → disposable-copy migration and application smoke → migration-checkout cleanup → final preflight → exact-SHA merge. Live migration still occurs only through normal application behavior after the merge and any declared service restart; pre-merge checks operate only on disposable database copies. Merge conflicts abort without automatic resolution.

`post_merge_tests` are different by design: every command runs with the exact staged production main as `cwd`, under the recorded staged repository fingerprint. They never run from the feature validation checkout.

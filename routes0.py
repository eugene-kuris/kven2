...
ROUTES_TOOLS_PROBE_VERSION = "owui-native-tools-bounded-decision-pseudocall-repair-2026-07-16-v10j"
...
async def _proxy_hybrid_final_response(
    ...
    *,
    ...
    """Proxy a final answer pass and safely schedule Kven write_path."""
    stream = bool(payload.get("stream", False))
    ...
    if stream:
        async def stream_backend():
            ...
            async with httpx.AsyncClient(timeout=timeout_seconds) as client:
                async with client.stream("POST", chat_url, json=payload) as response:
                    logger.info(
                        "[OWUI_NATIVE_HYBRID] backend_stream_status=%s content_type=%s",
                        response.status_code,
                        response.headers.get("content-type", ""),
                    )
                    ...
                    leftover = parse_buffer.rstrip("\r")
                    if leftover.startswith("data: "):
                        backend_lines.append(leftover)
    ...
    try:
        ...
        logger.info(
            ...
            "has_tool_calls=%s content_len=%s",
            response.status_code,
            choice0.get("finish_reason"),
            bool(message.get("tool_calls")),
            len(assistant_reply),
...
async def _forward_to_backend_and_collect(
    ...
    *,
    ...
    """
    Preserve the known-good baseline behavior: read the backend stream fully,
    collect SSE lines, then later return them through StreamingResponse.

    Tool-decision passes use a much smaller max_tokens plus a shorter timeout so
    an experimental tool loop cannot hang a user request indefinitely.
    """
    backend_chunks = []
    async with httpx.AsyncClient(timeout=timeout_seconds) as client:
        async with client.stream("POST", chat_url, json=payload) as response:
            ...
            if response.headers.get("content-type", "").startswith("text/event-stream"):
                ...
                logger.info(
                    f"[ROUTE] ✅ Stream completed. Total chunks: {len(backend_chunks)} route_label={route_label}"
                )
            else:
                logger.info(
                ...
                if content:
                    ...
                    logger.info(
                        f"[ROUTE] ✅ JSON fallback content extracted. Length: {len(content)} route_label={route_label}"
                    )
    ...
    try:
        ...
        if (
            ...
            if requested_call:
                ...
                backend_chunks = await _forward_to_backend_and_collect(
                    loop_payload,
                    chat_url,
